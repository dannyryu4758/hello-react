...
const serverRender = async (req, res, next) => {
  ...
  const jsx = (
    ...
    <PreloadContext.Provider value={preloadContext}>
      {/* 서버 리덕스 설정하기 (서버에서는 요청이 올때마다 스토어 다시 생성) */}
  ...
  const stateScript = `<script>__PRELOADED_STATE__ = ${stateString}</script>`; // redux 초기상태를 스크립트로 주입합니다.
  res.send(createPage(root, stateScript)); // 클라이언트에게 결과물로 응답합니다.
};
...
app.use(serve); // // JS, CSS 정적 파일 접근하기 / 순서가 중요함. serverRneder 전에 위치해야 한다.
